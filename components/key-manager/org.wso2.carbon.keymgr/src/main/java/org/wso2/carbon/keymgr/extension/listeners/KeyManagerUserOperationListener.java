 ...
 *
 */
package org.wso2.carbon.keymgr.extension.listeners;
...
import org.wso2.carbon.keymgr.extension.clients.ApiAuthenticationAdminClient;
...
public class KeyManagerUserOperationListener extends IdentityOathEventListener {
    ...
    public boolean doPreDeleteUser(String username, UserStoreManager userStoreManager) {
        ...
        try {
            ...
            //TODO: Clear workflow details from the event before delete user. Use HTTP call between IS and APIM to acheve this.
        ...
        KeyManagerApiUtils.clearRoleCache(getUserName(username, userStoreManager));
    ...
    private boolean removeGatewayKeyCache(String username, UserStoreManager userStoreManager) {
        ...
        //Part that extracted from IS method
        try {
            Set<AccessTokenDO> tokenDOS= accessTokenDAO.getAccessTokensOfUserStore(tenantId,userStoreDomain);
            for (AccessTokenDO entry : tokenDOS){
                activeTokens.add(entry.getAccessToken());
            }
    ...
    protected KeyManagerExtensionConfiguration getApiManagerConfiguration() {

        return KeyManagerServiceReferenceHolder.getInstance().getKeyManagerExtensionConfiguration();
    }
    ...
    protected Tenant getTenant(int tenantId) throws UserStoreException {
        ...
        return KeyManagerServiceReferenceHolder.getInstance().getRealmService().getTenantManager().getTenant(tenantId);
    }
    ...
    private String getUserName(String username, UserStoreManager userStoreManager) {
        ...
        if (!MultitenantConstants.SUPER_TENANT_DOMAIN_NAME.equalsIgnoreCase(tenantDomain) ||
                (!MultitenantUtils.isEmailUserName() &&
                        username.indexOf(KeyManagerConstants.EMAIL_DOMAIN_SEPARATOR) > 0)) {
            username = UserCoreUtil.addTenantDomainToEntry(username, tenantDomain);
        }
    ...
    private boolean invalidateMultipleCacheKeys(String[] users, UserStoreManager userStoreManager, boolean
        ...
        for (String username : users) {
            username = getUserName(username, userStoreManager);
            KeyManagerApiUtils.clearRoleCache(username);
            if (isEnable()) {
